...
## Castle Capital Security Standards

Castle Capital opera con capital real en DeFi (Uniswap, Revert), trading y asesoría
financiera. Estas reglas de seguridad son **obligatorias** y se aplican vía hooks
automáticos (ver `.claude/hooks/secrets-scanner.py`, `https-enforcer.py`,
`dependency-audit.py`).

### Credenciales y Secretos
- **Prohibido hardcodear:** API keys, private keys, passwords, tokens, bearer tokens
  o seed phrases en código fuente
- **Obligatorio:** Variables de entorno (`process.env.API_KEY`) o secret managers
- **Nunca commitear:** Archivos `.env` con valores reales (solo `.env.example` con
  placeholders como `YOUR_KEY_HERE`)

### Patrones Peligrosos (Prohibidos)
- `eval()`, `exec()`, `os.system()`, `subprocess.call(shell=True)` están **bloqueados**
- Usar `subprocess.run([...], shell=False)` con argumentos como lista explícita
- No construir comandos shell concatenando input de usuario

### HTTPS Obligatorio
- Todas las URLs a servicios externos **DEBEN** usar `https://`
- `http://` solo permitido para `localhost`, `127.0.0.1`, `0.0.0.0`, `::1`
- Aplica a `.js`, `.ts`, `.jsx`, `.tsx`, `.py`, `.json`, `.yaml`

### Datos Financieros y PII
- **Nunca loggear:** balances completos de wallet, direcciones completas, PII de
  clientes, private keys, montos exactos de transacciones
- **Obligatorio enmascarar:** wallets a últimos 4 caracteres (`0x...abcd`)
- Sanitizar logs antes de enviar a observabilidad

### Dependencias Externas
- **Aprobación explícita** requerida para cualquier paquete nuevo
- Preferir Node.js stdlib sobre dependencias
- Ejecutar `npm audit` antes de instalar paquete nuevo
- Paquetes con historial de supply chain attacks están **bloqueados**
  (`event-stream`, `flatmap-stream`, `ua-parser-js@0.7.29`)

### Criptografía
- Usar **solo librerías auditadas:** Node.js `crypto`, `ethers.js`, `@noble/*`
- **Prohibido:** implementaciones custom de crypto, hashing manual, RNG no-seguro
- Usar `crypto.randomBytes()`, nunca `Math.random()` para seguridad

### DeFi APIs (Uniswap / Revert)
- **Rate limiting** obligatorio en todas las llamadas
- **Retry logic** con backoff exponencial
- **Error handling** completo — nunca tragar excepciones silenciosamente
- **Nunca exponer** private keys directamente a APIs externas
- Usar wallet abstraction (signers de ethers.js) para firma local

### Validación de Inputs
- Sanitizar y validar **todo** input externo (user input, API responses, archivos)
- Usar schemas (`zod`, `joi`) para validación estructural
- Whitelist > blacklist para validación

> **Referencia formal:** `.claude/rules/security-mcp-usage.md`,
> `.claude/rules/castle-capital-security-agents.md`,
> Skill `/castle-capital-security` para auditoría completa
